...
ls -rt : 파일들 생성 시간순(제일 오래된 것부터)으로 표시

2. cd (Change directiory) : 디렉터리 이동
...
mkdir -p dirname/sub_dirname : dirname 생성 sub_dirname 하위 디렉터리 생성
...
rm -r dir : dir 디렉터리 삭제(디렉터리는 -r 옵견 없이 삭제 불가)

8. cat(Catenate) : 파일의 내용을 화면에 출력, 리다이렉션 기호('>')를 사용하여 새로운 파일 생성
cat file1 : file1의 내용을 출력
cat file1 file2 : file1과 file2의 내용을 출력
cat file1 file2 | more : file1과 file2의 내용을 페이지별로 출력
cat file1 file2 | head : file1과 file2의 내용을 처음부터 10번쨰 줄까지만 출력
cat file1 file2 | tail : file1과 file2의 내용을 끝에서부터 10번쨰 줄까지만 출력
...
head -n -5 ./test.log #위에서부터 5줄뺀 나머지 출력
